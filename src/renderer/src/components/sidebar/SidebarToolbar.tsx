...
function FeedbackDialog({
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    if (!trimmed) {
      toast.warning(t('sidebar.toolbar.feedback.toast.enterFeedback'))
      return
    }
    ...
    try {
      ...
      toast.success(t('sidebar.toolbar.feedback.toast.thanks'))
      setFeedback('')
      setSubmitAnonymously(false)
      onOpenChange(false)
    } catch (err) {
      toast.error(t('sidebar.toolbar.feedback.toast.submitFailed'))
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-sm">{t('sidebar.toolbar.feedback.title')}</DialogTitle>
          <DialogDescription className="text-xs">
            {t('sidebar.toolbar.feedback.description')}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-2 rounded-md border border-border/70 bg-muted/30 p-3">
          <div className="text-xs font-medium text-foreground">
            {t('sidebar.toolbar.feedback.otherWays')}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {t('sidebar.toolbar.feedback.githubIssues')}
            ...
            >
              ...
              {t('sidebar.toolbar.feedback.joinDiscord')}
              <ExternalLink className="size-3.5" />
            </Button>
            <Button
              type="button"
            ...
            >
              ...
              {t('sidebar.toolbar.feedback.followOnX')}
              <ExternalLink className="size-3.5" />
            </Button>
          </div>
        </div>
        ...
        <textarea
          ...
          placeholder={t('sidebar.toolbar.feedback.placeholder')}
          rows={7}
        ...
        <div className="min-h-9 rounded-md border border-border/70 bg-muted/30 px-3 py-2">
          {viewer ? (
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              <span>
                {t('sidebar.toolbar.feedback.githubLabel')}{' '}
              ...
              <label className="flex cursor-pointer items-center gap-2 text-foreground">
                ...
                />
                {t('sidebar.toolbar.feedback.submitAnonymously')}
              </label>
            </div>
          ) : isViewerLoading ? (
            <div className="text-xs text-muted-foreground">
              {t('sidebar.toolbar.feedback.checkingIdentity')}
            </div>
          ) : (
            <div className="text-xs text-muted-foreground">
              {t('sidebar.toolbar.feedback.submitOnlyHint')}
            </div>
          )}
        </div>
        <DialogFooter>
          ...
          <Button onClick={() => void handleSubmit()} disabled={isSubmitting || !feedback.trim()}>
            {isSubmitting
              ? t('sidebar.toolbar.feedback.sending')
              : t('sidebar.toolbar.feedback.send')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              ...
              <span className="text-[11px]">{t('sidebar.toolbar.addProject')}</span>
            </Button>
          </TooltipTrigger>
          <TooltipContent side="top" sideOffset={4}>
            {t('sidebar.toolbar.addProjectTooltip')}
          </TooltipContent>
        </Tooltip>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-xs"
                    type="button"
                    aria-label={t('sidebar.toolbar.toolboxAriaLabel')}
          ...
          <DropdownMenu modal={false}>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-xs"
                    type="button"
                    aria-label={t('sidebar.toolbar.helpAriaLabel')}
                    className="text-muted-foreground"
                  >
            ...
            <DropdownMenuContent side="top" align="start" sideOffset={8} className="w-48">
              ...
              >
                ...
                {t('sidebar.toolbar.showOnboarding')}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => setFeedbackOpen(true)}>
                <MessageSquareText className="size-3.5" />
                {t('sidebar.toolbar.sendFeedback')}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => openExternalUrl(DOCS_URL)}>
                <ExternalLink className="size-3.5" />
                {t('sidebar.toolbar.docs')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <Tooltip>
            ...
            <TooltipContent side="top" sideOffset={4}>
              {t('sidebar.toolbar.settingsTooltip')}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
